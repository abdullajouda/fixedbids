...
class GoogleMapPlacePicker extends StatelessWidget {
  ...
  _searchByCameraLocation(PlaceProvider provider) async {
    ...
    if (response.errorMessage != null ||response.errorMessage?.isNotEmpty == true ||
    ...
    if (usePlaceDetailSearch) {
      ...
      if (detailResponse.errorMessage != null ||detailResponse.errorMessage?.isNotEmpty == true ||
    ...
    return Selector<PlaceProvider, MapType>(
        ...
        builder: (_, data, __) {
          ...
          CameraPosition initialCameraPosition =
              CameraPosition(target: initialTarget, zoom: zoom ?? 15);
  ...
  Widget _buildFloatingCard() {
    return Consumer<PlaceProvider>(
      builder: (context, data, __) {
        if ((data.selectedPlace == null && data.placeSearchingState == SearchingState.Idle) ||
            data.isSearchBarFocused == true) {
          return Container();
        } else {
          if (selectedPlaceWidgetBuilder == null) {
            return _defaultPlaceWidgetBuilder(context, data.selectedPlace, data.placeSearchingState);
          } else {
            return Builder(
                builder: (builderContext) => selectedPlaceWidgetBuilder(
                    builderContext, data.selectedPlace, data.placeSearchingState, data.isSearchBarFocused));
          }
        }
      },
    );
